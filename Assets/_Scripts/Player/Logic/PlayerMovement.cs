...
{
    private bool _canMove = true, _isGrounded;
    ...
    public void PlayerMove(GameObject player, IPlayerInput playerInput)
    {
        ...
        float curSpeedX = _canMove ? (playerInput.playerRunInput ? _playerData.runSpeed : _playerData.walkSpeed) * playerInput.playerMovementInput.y : 0;
        float curSpeedY = _canMove ? (playerInput.playerRunInput ? _playerData.runSpeed : _playerData.walkSpeed) * playerInput.playerMovementInput.x : 0;
        float movementDirectionY = _moveDirection.y;
        _moveDirection = (forward * curSpeedX) + (right * curSpeedY);

        if (playerInput.playerJumpInput && _canMove && _isGrounded) _moveDirection.y = _playerData.jumpHeight;
