...
public class PlayerRotate : MonoBehaviour, IPlayerRotate
{
    ...
    public void RotatePlayer(GameObject player, IPlayerInput playerInput)
    {
        ...
        _rotationX = player.transform.localEulerAngles.y + Input.GetAxis("Mouse X") * _playerData.mouseSensitivityX * Time.deltaTime;
        _rotationY += Input.GetAxis("Mouse Y") * _playerData.mouseSensitivityY * Time.deltaTime;
        ...
        player.transform.localEulerAngles = new Vector3(0, _rotationX, 0);
        _playerData.camHolder.localEulerAngles = new Vector3(-_rotationY, 0, 0);

        print(playerInput.mouseMovementInput);
    }
}
